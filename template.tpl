...

{
  ...
  "description": "[Deprecated] Tag that send the event data from the Universal Analytics/GA4/Data client to Facebook Offline Conversion.",
  "containerContexts": [
    "SERVER"
  ]
}
...
[
  {
    "type": "LABEL",
    "name": "deprecated",
    "displayName": "Tag is deprecated because Meta deprecated Offline Conversions API. \u003ca href\u003d\"https://developers.facebook.com/docs/marketing-api/offline-conversions/\" target\u003d\"_blank\"\u003eMore information\u003c/a\u003e.  \n\u003cbr\u003e\nPlease use the \u003ca href\u003d\"https://stape.io/blog/facebook-offline-conversion-using-server-gtm#how-to-set-up-facebook-offline-conversions-using-s-gtm\" target\u003d\"_blank\"\u003enew method of tracking offline conversions\u003c/a\u003e.\n\u003cbr\u003e\u003cbr\u003e"
  },
  {
    "type": "RADIO",
    "name": "eventType",
...
function addEcommerceData(eventData, mappedData) {
  ...
  if (eventData.order_id) mappedData.order_id = eventData.order_id;
    ...
    mappedData.order_id = eventData.transaction_id;
...
function addUserData(eventData, mappedData) {
  ...
  else if (eventData.match_keys && eventData.match_keys.email_address)
    mappedData.match_keys.email = eventData.match_keys.email_address;
  else if (eventData.match_keys && eventData.match_keys.email)
    mappedData.match_keys.email = eventData.match_keys.email;

  if (eventData.phone) mappedData.match_keys.phone = eventData.phone;
  else if (eventData.match_keys && eventData.match_keys.phone_number)
    mappedData.match_keys.phone = eventData.match_keys.phone_number;
  ...
  if (eventData.lastName) mappedData.match_keys.ln = eventData.lastName;
    ...
    eventData.match_keys &&
    eventData.match_keys.address &&
    eventData.match_keys.address.last_name
  )
    mappedData.match_keys.ln = eventData.match_keys.address.last_name;
  ...
  if (eventData.firstName) mappedData.match_keys.fn = eventData.firstName;
    ...
    eventData.match_keys &&
    eventData.match_keys.address &&
    eventData.match_keys.address.first_name
  )
    mappedData.match_keys.fn = eventData.match_keys.address.first_name;
  ...
  if (eventData.city) mappedData.match_keys.ct = eventData.city;
    ...
    eventData.match_keys &&
    eventData.match_keys.address &&
    eventData.match_keys.address.city
  )
    mappedData.match_keys.ct = eventData.match_keys.address.city;

  if (eventData.state) mappedData.match_keys.st = eventData.state;
  else if (
    eventData.match_keys &&
    eventData.match_keys.address &&
    eventData.match_keys.address.region
  )
    mappedData.match_keys.st = eventData.match_keys.address.region;

  if (eventData.zip) mappedData.match_keys.zip = eventData.zip;
  else if (
    eventData.match_keys &&
    eventData.match_keys.address &&
    eventData.match_keys.address.postal_code
  )
    mappedData.match_keys.zip = eventData.match_keys.address.postal_code;

  if (eventData.countryCode)
    mappedData.match_keys.country = eventData.countryCode;
  else if (eventData.country) mappedData.match_keys.country = eventData.country;
  else if (
    eventData.match_keys &&
    eventData.match_keys.address &&
    eventData.match_keys.address.country
  )
    mappedData.match_keys.country = eventData.match_keys.address.country;

  if (eventData.madid) mappedData.match_keys.madid = eventData.madid;

  if (eventData.external_id)
    mappedData.match_keys.external_id = eventData.external_id;
  else if (eventData.user_id)
    mappedData.match_keys.external_id = eventData.user_id;
  else if (eventData.userId)
    mappedData.match_keys.external_id = eventData.userId;
